...
export function StylePreview({ type }: StylePreviewProps) {
  ...
  switch (type) {
    case 'minimalist':
      return <div className="w-16 h-16 bg-white border border-gray-200 shadow-sm" />;
    case 'swiss':
      return (
        <div className="w-full h-full p-4 flex flex-col justify-between border-t-4 border-l-4 border-red-600 bg-white">
          <div className="h-2 w-1/2 bg-black mb-1" />
    ...
    case 'neobrutalism':
      return (
        <div className="w-20 h-12 bg-[#B1FF5B] border-4 border-black" style={{ boxShadow: '4px 4px 0 0 #000' }}>
          <div className="h-full w-full flex items-center justify-center font-mono font-bold text-black text-xs">BTN</div>
        </div>
      );
    case 'glassmorphism':
      return (
        <div className="w-full h-full flex items-center justify-center bg-gradient-to-tr from-pink-300 to-indigo-400 p-4">
          <div className="w-full h-full bg-white/30 backdrop-blur-md rounded-xl border border-white/40 shadow-xl" />
        </div>
      );
    case 'terminal':
      return (
        <div className="w-full h-full bg-black p-4 font-mono text-[10px] text-green-500 leading-tight flex flex-col justify-end">
          <div>$ init design</div>
    ...
    case 'editorial':
      return (
        <div className="w-full h-full bg-[#f4ebd0] p-4 flex flex-col">
          <div className="border-b border-black pb-1 mb-2">
            <h1 className="font-serif text-lg leading-none text-black">The Edit</h1>
          </div>
          <div className="flex gap-2">
            <div className="w-1/3 h-10 bg-black/10" />
            <div className="flex-1 flex flex-col gap-1 mt-1">
              <div className="h-1 w-full bg-black/40" />
              <div className="h-1 w-5/6 bg-black/40" />
              <div className="h-1 w-4/6 bg-black/40" />
            </div>
          </div>
        </div>
      );
    case 'bauhaus':
      return (
        <div className="w-full h-full bg-[#E5DCC5] flex items-center justify-center relative overflow-hidden">
          <div className="absolute w-16 h-16 bg-[#D83121] rounded-full -top-4 -left-4" />
          <div className="absolute w-12 h-12 bg-[#2D56A0] -bottom-2 right-4" />
          <div className="absolute w-0 h-0 border-l-[20px] border-l-transparent border-r-[20px] border-r-transparent border-b-[34.6px] border-[#F2C029] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" />
        </div>
      );
    case 'y2k':
      return (
        <div className="w-full h-full bg-black flex items-center justify-center p-4">
          <div className="text-xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-zinc-100 via-zinc-400 to-zinc-800 drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">
            CYBER
          </div>
        </div>
      );
